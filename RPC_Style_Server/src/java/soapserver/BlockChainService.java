...
public class BlockChainService {
    ...
    public void addBlock(@WebParam(name = "payload") String payload) {            
        String[] twoParts = payload.split("#");
        // verify client identity using its RSA public key
        boolean isValidClient = verifyClient(twoParts);
        if(!isValidClient){
            return;
        }                
        // read in data and difficulty in plain payload
        String[] realData = twoParts[0].split("\t");
        String data = realData[0];        
        int difficulty = Integer.parseInt(realData[1]);          
